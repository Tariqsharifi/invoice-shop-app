...
const InvoiceDetail = {
  ...
  renderItemRow(it) {
    return `
      <div class="history-row" data-purchase-id="${it.id}" data-product-id="${it.product_id ?? it.products?.id ?? ""}">
    ...
    row.innerHTML = `
      <div class="item-edit-form">
        <div class="item-edit-field">
          ...
          <input type="text" class="edit-name" value="${escapeHtml(item.products?.name ?? "")}" />
        </div>
        <div class="item-edit-field">
          <label>تعداد</label>
  ...
  async saveItem(purchaseId, invoiceId) {
    ...
    if (!name) {
      showToast("نام کالا نمی‌تونه خالی باشه", "error");
      return;
    }

    setLoading(true, "در حال ذخیره…");
    try {
      await DB.setPurchaseItemProductName(purchaseId, name);
