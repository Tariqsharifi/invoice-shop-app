...
const OCR = {
   ...
   * فاکتور کپی کرده رو پردازش می‌کنه. Live Text معمولاً هر عدد/اسم رو تو یک
   * خط جدا کپی می‌کنه (نه همه با هم تو یک خط)، پس اینجا خط‌به‌خط جمع می‌کنیم:
   * اسم‌ها پشت‌سرهم جمع می‌شن تا به اولین بلوک از حداقل ۲ عدد برسیم، همون
   * منطق «تعداد × قیمت‌واحد ≈ مبلغ کل» رو پیاده می‌کنیم، و می‌ریم سراغ آیتم بعدی.
   */
  parsePastedText(text) {
    ...
    // اگه متن دقیقاً با فرمت «اسم | تعداد | قیمت‌واحد | مبلغ‌کل» اومده باشه
    // (مثلاً وقتی از یه هوش مصنوعی که خودِ عکس رو دیده کپی کردی)، نیازی به
    // حدس‌زدن نیست — مستقیم و بدون خطا می‌خونیمش.
